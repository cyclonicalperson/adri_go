...
export class SiteTranslateService implements OnDestroy {
  ...
  private readonly storageKey = 'adrigo_admin_language';
  private readonly defaultLanguage: SiteLanguageCode = 'sr';
  ...
  private translateText(text: string): string {
    if (this.currentLanguage === this.defaultLanguage || !text.trim()) {
      return text;
    }
    ...
    for (const [source, target] of Object.entries(this.dictionary).sort((a, b) => b[0].length - a[0].length)) {
      if (!source.trim() || source.startsWith('__') || !partial.includes(source)) {
        continue;
      }
