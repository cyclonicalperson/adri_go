...
internal static class TourismTools
{
    ...
    [Description(
        ...
        "Types: city, mountain, lake, national_park, coast, village, other. " +
        "Country filter accepts 'Montenegro' or 'Serbia'. " +
        "Use query to search by name, e.g. 'Zabljak', 'Durmitor', 'Budva'.")]
    ...
    [Description(
        ...
        "Provide the region name (e.g. 'Zabljak', 'Durmitor', 'Kotor') — IDs are resolved automatically.")]
    ...
    [Description(
        ...
        "Use regionName to filter by destination (e.g. 'Zabljak', 'Budva'). " +
        "Use tags to filter by amenity or feature, e.g. ['bazen', 'spa', 'wellness', 'parking', 'wifi']. " +
        "Use query for free-text search including amenities listed in location details (e.g. 'spa', 'bazen', 'pool').")]
    public static async Task<PagedResult<PostSummary>> SearchPosts(
        ITourismQueryService tourismService,
        CancellationToken cancellationToken,
        [Description("Region name or partial name (e.g. 'Zabljak', 'Budva'). Resolved automatically.")] string? regionName = null,
        [Description("Optional free-text query matching title, description, address or amenities (e.g. 'spa', 'bazen', 'pool').")] string? query = null,
        ...
        [Description("Optional tag/amenity names to filter by, e.g. ['bazen', 'spa', 'planinarenje', 'parking', 'wifi'].")] IReadOnlyList<string>? tags = null,
    ...
    [Description(
        ...
        "Provide the location name (e.g. 'Hotel Durmitor', 'Restoran Jezero') and the ID is resolved automatically. " +
        "Returns null if the location is not found or not published.")]
    public static async Task<PostDetail?> GetPostDetail(
        ITourismQueryService tourismService,
        CancellationToken cancellationToken,
        [Description("Location name or partial name (e.g. 'Hotel Durmitor', 'Restoran Jezero').")] string? postName = null)
    {
    ...
    [Description(
        ...
        "Use regionName to filter by destination (e.g. 'Durmitor', 'Kotor').")]
    public static async Task<PagedResult<RouteSummary>> SearchRoutes(
        ITourismQueryService tourismService,
        CancellationToken cancellationToken,
        [Description("Region name or partial name (e.g. 'Durmitor', 'Zabljak'). Resolved automatically.")] string? regionName = null,
    ...
    [Description(
        "Get full details for a specific route: waypoints, distance, duration, elevation gain, view count, save count. " +
        "Provide the route name (e.g. 'Crno jezero krug') and the ID is resolved automatically. " +
        "Returns null if the route is not found or not published.")]
    public static async Task<RouteDetail?> GetRouteDetail(
        ITourismQueryService tourismService,
        CancellationToken cancellationToken,
        [Description("Route name or partial name (e.g. 'Crno jezero', 'Bobotov Kuk').")] string? routeName = null)
    {
        var resolvedId = await ResolveRouteId(null, routeName, tourismService, cancellationToken);
        if (resolvedId is null) return null;
    ...
    [Description(
        ...
        "Provide the route name (e.g. 'Crno jezero') and the ID is resolved automatically. " +
    ...
    [Description(
        "Search published events (concerts, festivals, sports, theatre, exhibitions, tours...). " +
        "Returns all upcoming and past events when no date filter is applied. " +
        "Categories: CONCERT, SPORT, THEATER, FESTIVAL, EXHIBITION, OTHER. " +
        "Use regionName to filter by destination (e.g. 'Zabljak', 'Budva'). " +
        "Date filters: provide startFrom and/or startTo as ISO 8601 strings (e.g. '2025-07-01T00:00:00'). " +
    ...
    public static async Task<PagedResult<EventSummary>> SearchEvents(
        ...
        [Description("Optional category filter: CONCERT, SPORT, THEATER, FESTIVAL, EXHIBITION, OTHER.")] string? category = null,
    ...
    public static async Task<IReadOnlyList<PostSummary>> GetSimilarPosts(
        ITourismQueryService tourismService,
        CancellationToken cancellationToken,
        [Description("Location name or partial name (e.g. 'Restoran Soa', 'Hotel Jezera').")] string? postName = null,
    ...
    [Description(
        ...
        "Date format: 'yyyy-MM-dd' or ISO 8601 (e.g. '2025-01-01' or '2025-01-01T00:00:00').")]
    public static async Task<IReadOnlyList<VisitTrendPoint>> GetVisitTrends(
        ITourismQueryService tourismService,
        CancellationToken cancellationToken,
        [Description("Region name or partial name (e.g. 'Zabljak'). Resolved automatically.")] string? regionName = null,
        [Description("Start date (e.g. '2025-01-01'). Defaults to 30 days ago.")] string? fromDate = null,
        [Description("End date (e.g. '2025-12-31'). Defaults to today.")] string? toDate = null)
    {
        var resolvedRegionId = await ResolveRegionId(null, regionName, tourismService, cancellationToken);
        ...
        if (!string.IsNullOrWhiteSpace(fromDate) &&
            DateTime.TryParse(fromDate, System.Globalization.CultureInfo.InvariantCulture,
                System.Globalization.DateTimeStyles.AssumeUniversal | System.Globalization.DateTimeStyles.AdjustToUniversal,
                out var parsedFrom))
            from = parsedFrom;

        if (!string.IsNullOrWhiteSpace(toDate) &&
            DateTime.TryParse(toDate, System.Globalization.CultureInfo.InvariantCulture,
                System.Globalization.DateTimeStyles.AssumeUniversal | System.Globalization.DateTimeStyles.AdjustToUniversal,
                out var parsedTo))
            to = parsedTo;

        return await tourismService.GetVisitTrendsAsync(
            new GetVisitTrendsRequest(resolvedRegionId, from, to),
            cancellationToken);
    }
    ...
    [Description(
        ...
        "Use locationName and/or regionName to filter results.")]
    public static async Task<IReadOnlyList<ExternalClickSummary>> GetExternalClickStats(
        ITourismQueryService tourismService,
        CancellationToken cancellationToken,
        [Description("Optional: filter by a specific location's name (e.g. 'Hotel Jezero').")] string? locationName = null,
        [Description("Region name or partial name (e.g. 'Zabljak'). Resolved automatically.")] string? regionName = null,
        [Description("Maximum number of results (default 20).")] int? limit = null)
    {
        var resolvedPostId   = await ResolvePostId(null, locationName, tourismService, cancellationToken);
        var resolvedRegionId = await ResolveRegionId(null, regionName, tourismService, cancellationToken);
        ...
        [Description("Number of results to skip for pagination (default 0).")] int? offset = null)
    {
        ...
        return tourismService.GetSavedPostsAsync(
            new GetSavedPostsRequest(null, limit ?? 20, offset ?? 0),
            cancellationToken);
    }
        ...
        [Description("If true, returns only publicly shared planners (default false).")] bool? onlyPublic = null)
    {
        ...
        return tourismService.GetTouristPlannersAsync(
            new GetTouristPlannerRequest(null, onlyPublic ?? false),
            cancellationToken);
    }
        ...
        [Description("Number of results to skip for pagination (default 0).")] int? offset = null)
    {
        ...
        return tourismService.GetTouristFavoritesAsync(
            new GetTouristFavoritesRequest(null, entityType, limit ?? 20, offset ?? 0),
            cancellationToken);
    }
