...
public sealed class AiTourismQueryService : IAiTourismQueryService
{
    private readonly AppDbContext _db;
    private readonly ILogger<AiTourismQueryService> _logger;

    public AiTourismQueryService(AppDbContext db, ILogger<AiTourismQueryService> logger)
    {
        _db = db;
        _logger = logger;
    }
        ...
        SearchPostsRequest request, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.Query))
        {
            ...
            query = query.Where(x =>
                ...
                (x.Details != null && EF.Functions.ILike(x.Details, $"%{s}%")));
        }
        ...
        {
            // Pretražujemo po tagu (naziv taga) ILI po ključnoj rijeci u details JSON-u
            // Na taj način "bazen" pronalazi i lokacije čiji details sadrže "pool"/"bazen"
            // a i lokacije s direktnim tag-om tog naziva
            var tagNames = request.Tags.Select(t => t.ToLower()).ToList();
            query = query.Where(x =>
                x.PostTags.Any(pt => tagNames.Contains(pt.Tag.Name.ToLower())) ||
                tagNames.Any(t => x.Details != null && EF.Functions.ILike(x.Details, $"%{t}%")) ||
                tagNames.Any(t => x.Description != null && EF.Functions.ILike(x.Description, $"%{t}%")));
        }
        ...
        PostDetailRequest request, CancellationToken cancellationToken)
    {
        ...
        // DbContext nije thread-safe, zato se dodatni upiti izvršavaju sekvencijalno.
        var regionName = post.RegionId.HasValue
        ...
        var viewCount = await _db.PostViews.AsNoTracking()
            .CountAsync(x => x.PostId == post.Id, cancellationToken);
        var likeCount = await _db.PostLikes.AsNoTracking()
            .CountAsync(x => x.PostId == post.Id, cancellationToken);
        ...
        GetPostAnalyticsRequest request, CancellationToken cancellationToken)
    {
        ...
        var reviews = await _db.Reviews.AsNoTracking()
            .Where(x => x.PostId.HasValue && postIds.Contains(x.PostId.Value) && x.IsApproved)
            .GroupBy(x => x.PostId!.Value)
            .Select(g => new { PostId = g.Key, Count = g.Count() })
            .ToListAsync(cancellationToken);
        ...
        uint touristId, CancellationToken cancellationToken)
    {
        ...
        if (reviews.Count > 0)
        {
            ...
            var reviewedPosts = await _db.Posts.AsNoTracking()
                ...
                .ToListAsync(cancellationToken);
            var reviewsByPostId = reviews
                .Where(r => r.PostId.HasValue)
                .ToDictionary(r => r.PostId!.Value);
            foreach (var p in reviewedPosts)
            {
                var weight = reviewsByPostId.TryGetValue(p.Id, out var r) ? (decimal)Math.Max((int)r.Rating, 1) : 1m;
    ...
    private static void AddPostSignal(PreferenceProfile profile, Post post, decimal weight)
    {
    ...
    private static RecommendationItem ScoreAnonymousPost(Post post, string mode)
    {
    ...
    private static RecommendationItem ScoreAnonymousRoute(TouristGuide.Api.Models.Route route)
    {
    ...
    private static RecommendationItem ScorePersonalizedPost(PreferenceProfile profile, Post post, string mode)
    {
    ...
    private static RecommendationItem ScorePersonalizedRoute(PreferenceProfile profile, TouristGuide.Api.Models.Route route)
    {
    ...
    private static decimal CalculatePostPopularity(Post post)
    {
        var rating = Math.Min(post.AvgRating ?? 0m, 5m) / 5m;
        return rating;
    }

    private static decimal CalculateRoutePopularity(TouristGuide.Api.Models.Route route)
    {
        ...
        GetRegionAnalyticsRequest request, CancellationToken cancellationToken)
    {
        ...
        var viewsCount = await _db.PostViews.AsNoTracking()
            .CountAsync(x => postIds.Contains(x.PostId), cancellationToken);
        var likesCount = await _db.PostLikes.AsNoTracking()
            .CountAsync(x => postIds.Contains(x.PostId), cancellationToken);
        var sharesCount = await _db.ContentShares.AsNoTracking()
        ...
        GetVisitTrendsRequest request, CancellationToken cancellationToken)
    {
        ...
        return rawViews
            .GroupBy(d => d.Date)
            .OrderBy(g => g.Key)
            .Select(g => new VisitTrendPoint(g.Key.ToString("yyyy-MM-dd"), g.Count()))
            .ToList();
    }
    ...
    /// <summary>Mapira AI kategoriju aktivnosti na ključne riječi za pretragu u bazi.</summary>
    private static IReadOnlyList<string> MapActivityCategoryToKeywords(string category) =>
        category switch
        {
            "SPORT"       => new[] { "sport", "skijanje", "plivanje", "biciklizam", "trčanje" },
            "ADVENTURE"   => new[] { "adrenalin", "rafting", "kanjoning", "zipline", "bungee", "paraglajding" },
            "WELLNESS"    => new[] { "wellness", "spa", "yoga", "masaža", "sauna" },
            "SHOPPING"    => new[] { "shopping", "suvenirnica", "tržnica", "kupovina" },
            "DINING"      => new[] { "gastronomija", "hrana", "restoran", "degustacija" },
            "NIGHTLIFE"   => new[] { "noćni život", "muzika", "klub", "bar", "party" },
            "SIGHTSEEING" => new[] { "razgledanje", "tura", "vodič", "panorama" },
            "CULTURE"     => new[] { "kultura", "muzej", "galerija", "historijsko" },
        ...
        SearchActivitiesRequest request, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.Category))
        {
            // Mapiranje AI kategorija (SPORT, ADVENTURE, WELLNESS...) na tag nazive/opise
            // Budući da tagovi nemaju Color prefiks u bazi, radimo fuzzy match po imenu i opisu
            var cat = request.Category.Trim().ToUpper();
            var categoryKeywords = MapActivityCategoryToKeywords(cat);

            if (categoryKeywords.Count > 0)
            {
                query = query.Where(x =>
                    categoryKeywords.Any(kw =>
                        EF.Functions.ILike(x.Name, $"%{kw}%") ||
